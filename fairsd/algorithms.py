...
class ResultSet:
    ...
    def sg_feature(self, sg_index, X):
        ...
        if sg_index >= len(self.descriptions_list) or sg_index < 0:
            raise RuntimeError("The requested subgroup doesn't exists")
        return pd.Series(
            self.descriptions_list[sg_index].to_boolean_array(X), name=str("sg" + str(sg_index))
        )
    ...
    def to_json(self, X: pd.DataFrame, metric: str, result_set_df: pd.DataFrame):
        ...
        return {"descriptions": descriptions, "sg_features": sg_features, "metric": metric}
...
class DSSD:
    ...
    def beam_creation(
        self, tuples_sg_matrix, support_array, quality_array, decriptions_list, beam, beam_width
    ):
        """
        Parameters
        ----------
